...
void keyboard_down(int evkeyboardkeycode,int *andou_b,int *andou_c,int *andou_d,int *andou_e,int *correr,bool *puxa,int *tlep,int *cx,int *cy,Magia (*fireball)[2],int *energia)
{
		for(int i=0; i<4;i++)
	{
		if(pessoa[i].comp==0)
		{
			if(evkeyboardkeycode==pessoa[i].botao_char_int[0])
			{
				andou_c[i] = 1;
			}
			if(evkeyboardkeycode==pessoa[i].botao_char_int[1])
			{
				andou_b[i] = 1;
			}
			if(evkeyboardkeycode==pessoa[i].botao_char_int[2])
			{
				andou_d[i] = 1;
			}
			if(evkeyboardkeycode==pessoa[i].botao_char_int[3])
			{
				andou_e[i] = 1;
			}
			if(evkeyboardkeycode==pessoa[i].botao_char_int[4])
			{
				correr[i] = 2;
			}
			if(evkeyboardkeycode==pessoa[i].botao_char_int[5])
			{
				puxa[i] = true;
			}
			if(evkeyboardkeycode==pessoa[i].botao_char_int[6])
			{
				tlep[i] = 1;
			}
		}
...
void keyboard_up(int evkeyboardkeycode,int *andou_b,int *andou_c,int *andou_d,int *andou_e,int *correr,bool *puxa,bool *sair)
{
	for(int i=0; i<4;i++)
	{
		if(pessoa[i].comp==0)
