...
void IA(int *andou_b,int *andou_c,int *andou_d,int *andou_e)
{
	...
	for(int i=0;i<4;i++)
	{
		if((pessoa[i].comp==1)and(pessoa[pessoa_perto[i]].time!=pessoa[i].time))
		{
			if((pessoa[pessoa_perto[i]].x-pessoa[i].x==0)and(pessoa[pessoa_perto[i]].y-pessoa[i].y>0))
			{
				andou_b[i]=1;andou_e[i]=0;andou_d[i]=0;andou_c[i]=0;
			}else if((pessoa[pessoa_perto[i]].x-pessoa[i].x==0)and(pessoa[pessoa_perto[i]].y-pessoa[i].y<0))
			{
...
int main()
{
	...
	for(int r=0;r<NJOGADORES;r++){
		andou_b[r]=0;andou_c[r]=0;andou_d[r]=0;andou_e[r]=0;correr[r]=1;energia[r]=100;
		desx[r]=64;desy[r]=0;
	}
	Window win;
	...
	if(!errext) exit(1);		// Se deu erro no lugar onde vao ser anotados os erros, ferrou!
